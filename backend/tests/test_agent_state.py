...
from app.agents.types import WorkflowStatus, AgentResult, AgentStatus
...
def test_initial_state_auto_generates_request_id():
    ...
    assert state.request_id and len(state.request_id) > 0

def test_initial_state_uses_provided_request_id():
    state = create_initial_state(user_message="test", request_id="req-123")
    assert state.request_id == "req-123"

def test_initial_state_preserves_student_id():
    state = create_initial_state(user_message="test", student_id=42)
...
def test_initial_state_custom_max_steps():
    state = create_initial_state(user_message="test", max_steps=5)
...
def test_empty_request_id_raises_error():
    with pytest.raises(Exception):
        AgentState(request_id="", user_message="valid message")

def test_is_step_limit_reached_false_at_start():
    state = create_initial_state(user_message="test", max_steps=10)
    assert state.is_step_limit_reached() is False

def test_is_step_limit_reached_true_when_at_limit():
    state = create_initial_state(user_message="test", max_steps=3)
    state.step_count = 3
