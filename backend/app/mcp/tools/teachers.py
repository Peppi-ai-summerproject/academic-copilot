...
def get_teacher(teacher_id: int) -> dict[str, Any]:
    ...
    try:
        return TeacherService(TeacherRepository(session)).get_teacher(teacher_id)
    except Exception:
...
def search_teachers(query: str | None = None, limit: int = 20, offset: int = 0) -> dict[str, Any]:
    ...
    session = SessionLocal()
    try:
        return TeacherService(TeacherRepository(session)).search_teachers(query, limit, offset)
    except Exception:
