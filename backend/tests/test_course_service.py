...
def test_search_courses_returns_multiple_candidates_and_empty_results() -> None:
    repository = Mock()
    repository.search_courses.return_value = ([{"id": 1}, {"id": 2}], 2)
    result = CourseService(repository).search_courses("SOFTWARE")
    assert result["success"] is True
    assert result["pagination"]["total"] == 2
    repository.search_courses.assert_called_once_with("SOFTWARE", 20, 0)
    repository.search_courses.return_value = ([], 0)
